...
for (d in 1:2) { # loop through sex d <> b
	for (j in 1:(nRg^2)) {
		...
		cArray[,,j,d,2] <- sweep(cArray[,,j,d,1], 1, popA, '*') / 
					     t(sweep(cArray[,,iid[j],b,1], 1, popB, '*'))
	}
}
...
for (j in 1:(nRg*2)) { # in each group and sex, count H+ and on art * reduction
	...
	Hsag  <- colSums(sweep(Hstar, 1, mx$wD[,3], '*')) # weighted by CD4, ∑ over
...
for (d in 1:2) { # loop through sex 
	for (j in 1:(nRg^2)) { # through all sex * group
		b <- ifelse(d==1, 2, 1)
		cArray[,,j,d,1] <- sweep(cArray[,,j,d,1], 2, 
...
# -----------------------------------------------------------------------------
if (mx$n==2) {
	cArray[,,,fp$ss$m.idx,1] <- sweep(cArray[,,,fp$ss$m.idx,1], 3, 
		c(t(mx$betaPair[,,fp$ss$m.idx])), '*')
	cArray[,,,fp$ss$f.idx,1] <- sweep(cArray[,,,fp$ss$f.idx,1], 3, 
		c(t(mx$betaPair[,,fp$ss$f.idx])), '*')
}

# Summing over other ages, groups
FOI <- array(0, c(fp$ss$pAG, fp$ss$NG))
if (mx$n == 1) { # TODO: find way for auto indexing
