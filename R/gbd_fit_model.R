...
#'   
prepare_spec_fit_gbd <- function(loc, collapse, k, proj.end=2016.5, gbd.pop = TRUE, popadjust = NULL, popupdate=TRUE, use_ep5=FALSE){
  
  ## TODO: Decide where to store pjnz
  pjnz <- system.file('inst/extdata/testpjnz/', paste0(loc, ".PJNZ"), package="eppasm")
  ## epp
  if(!collapse){
  ...
  if(gbd.pop){
    demp <- sub.pop.params.demp(demp, loc, k)
  }
