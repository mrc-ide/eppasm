...
plot_output <- function(fit, fit2=NULL, fit3=NULL, specres=NULL, cols=c("navy", "darkred", "darkgreen"), pages=1:3, likdat=NULL,
  ...
  if(1 %in% pages){
    ...
    mtext("HIV prevalence, age 15-49y", 3, 0.5, font=2, cex=1.1)
    ##
    ## incidence trend
    plot_incid2(fit, fit2, fit3, col=cols)
    if(!is.null(specres))
      lines(as.numeric(names(incid(specres))), incid(specres), lty=2, lwd=2, col="grey10")
    mtext("HIV incidence rate, age 15-49y", 3, 0.5, font=2, cex=1.1)
    ##
    ## tranmission rate
    plot_log_transmrate(fit, fit2, fit3, col=cols)
    if(!is.null(specres))
      lines(as.numeric(names(incid(specres))), log(incid(specres) / prev(specres)), lty=2, lwd=2, col="grey10")
    mtext("Log transmission rate", 3, 0.5, font=2, cex=1.1)
    ## 
    ## Sex ratio of incidence
    plot_incidsexratio(fit, fit2, fit3, col=cols)
    if(!is.null(specres))
      lines(as.numeric(names(incid_sexratio(specres))), incid_sexratio(specres), lty=2, lwd=2, col="grey10")
    mtext("Incidence sex ratio (F:M, 15-49y)", 3, 0.5, font=2, cex=1.1)
    ##
    ## ART coverage age 15+
    plot_artcov15plus(fit, fit2, fit3, col=cols)
    if(!is.null(specres) && exists("artnum.m", specres))
      lines(as.numeric(names(artcov15plus(specres))), artcov15plus(specres), lty=2, lwd=2, col="grey10")
    mtext("ART coverage, age 15+", 3, 0.5, font=2, cex=1.1)
    mtext(paste0(title, "; posterior distribution"), 3, 0.5, outer=TRUE, font=2, cex=1.3)
    ##
    ## Pregnant women prevalence
    if(!is.null(fit$pregprev)){
      plot_pregprev(fit, fit2, fit3, col=cols, likdat=likdat)
      if(!is.null(specres))
        lines(as.numeric(names(fnPregPrev(specres))), fnPregPrev(specres), lty=2, lwd=2, col="grey10")
      mtext("Prevalence among pregnant women", 3, 0.5, font=2, cex=1.1)
    }
  }
  ##
  ...
  if(2 %in% pages){
    ...
    mtext(paste0(title, "; Age-specific prevalence"), 3, 0.5, outer=TRUE, font=2, cex=1.3)
  }
  ##
  ##
  ...
  if(3 %in% pages){
    ...
    for(iagegr in 1:3){
      for(isex in 1:2){
        ...
        lines(xx, fit.yprev[,"mean"], col=cols[1], lwd=2)
        if(!is.null(fit2))
          lines(xx, fit2.yprev[,"mean"], col=cols[2], lwd=2)
        if(!is.null(fit3))
          lines(xx, fit3.yprev[,"mean"], col=cols[3], lwd=2)
        ##
        if(!is.null(specres)){
