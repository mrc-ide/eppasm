...
$P_{sag}$ is the tranmission probability, and $K_{Aa}$ is the probability of correct
and consistent condom usage given the pair. All the model's terms are
time-dependent but $K_{Aa}$ might not, depending on the availability of data.
...
```{r}
mx <- list()
mx$gamma <- c(.8, .2) # order Low -> High Risk, sum to 1
```
...
> TODO: add relavtive transmission rate for pair of risk group. This should go to transmission rate section.
...
\[
P_{sag} = r_t \cdot
\dfrac{\sum_{d} w_{d} \left[H_{sag}^{d,k=0} - H_{sag}^{d,k=1} (1 - \phi) \right] }{N_{sag}} = r_t \cdot \dfrac{H^{*}_{sag}}{N_{sag}}
\]
...
But as the HIV population is grouped, $H^{x}_{sag}$ is calculated by age-group and distributed as
...
The later years are slightly differrence after seventh digit. This should be the result of correcting the negative target population inputs. But it is important for the mixing model as the sign of population messing with FOI calculation.

### Mixing model

Model with `length(mx$gamma)` risk groups,
...
Change $\gamma$ to run 1 risk group
