...
void calc_infections_eppspectrum(const multi_array_ref<double, 4> pop, const multi_array_ref<double, 4> hivpop, const multi_array_ref<double, 5> artpop,
                                 int eppmod, double *rspline_rvec,
				 double rtrend_tstab, const double *rtrend_beta, double rtrend_r0, double relinfectART, double tsEpidemicStart, double iota,
                                 double *incrr_sex, const multi_array_ref<double, 3> incrr_age,
                                 const double *projsteps, int t_ART_start, double DT, int t, int hts, int ts,
                                 double *rvec, double *prevlast, double *prevcurr, double *incrate15to49_ts, double infections_ts[NG][pAG]);
...
extern "C" {
  ...
  SEXP spectrumC(SEXP s_fp){
	...
	calc_infections_eppspectrum(pop, hivpop, artpop,
                                    eppmod, rspline_rvec,
				    rtrend_tstab, rtrend_beta, rtrend_r0, relinfectART, tsEpidemicStart, iota,
                                    incrr_sex, incrr_age,
                                    projsteps, t_ART_start, DT, t, hts, ts,
                                    rvec, &prevlast, &prevcurr, &incrate15to49_ts_out[ts], infections_ts);
                                 ...
                                 double *rvec, double *prevlast, double *prevcurr, double *incrate15to49_ts, double infections_ts[NG][pAG])
{
  ...
  *incrate15to49_ts = rvec[ts] * Xhivp * (1.0 - (1.0 - relinfectART) * prop_art_ts) / Xtot + ((projsteps[ts] == tsEpidemicStart) ? iota : 0.0);
