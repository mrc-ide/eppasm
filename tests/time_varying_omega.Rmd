...
#pjnz <- paste0(spec_files, "burkina_faso_2022_v3.PJNZ")
...
inc1 <- apply(attr(mod,"infections"),3,sum)
inc2 <- apply(attr(mod,"infections"),3,sum)
...
plot(inc1[1:49],type="l")
lines(inc2[1:49],col="red")
...
theta_ur = apply(readRDS("~/HIV/tests/SEN_test1FSW.RDS")$`Travailleurs du sexe`$resample,2,mean)
locfit$`Travailleurs du sexe`<- eppasm::fitmod(loc$`Travailleurs du sexe`, eppmod = "rhybrid", rw_start = 2005,B0=1e3, B=1e2, opt_iter = 1, number_k=250, optfit = TRUE, opthess=FALSE, opt_init = theta_ur)
saveRDS(locfit,"~/HIV/tests/NIGER_test1FSW.RDS")
...
locfit$`Pop féminine restante`<- eppasm::fitmod(loc$`Pop féminine restante`,  eppmod = "rhybrid",  rw_start = 2005,B0=1e3, B=1e2, opt_iter = 1, number_k=250, optfit = TRUE, opthess=FALSE,  opt_init = theta_ur)
