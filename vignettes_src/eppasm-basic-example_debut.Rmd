...
```{r}
# CD4 mortality rate > 1
fp_par$cd4_mort %>% range
fp_par$cd4_mort %<>% `/`(max(fp_par$cd4_mort))
# Target pop < 0
...
```{r echo=FALSE, fig.height=5, fig.cap = "Figure: Prevalence 15-49"}
put(1,1, c(5,4,1,1))
...
```{r echo=FALSE, fig.height=5, fig.cap = "Figure: Prevalence by ages (no debut)"}
put(1,1, c(5,4,1,1))
plot_prev(modR, main="no debut")
```

```{r echo=FALSE, fig.height=5, fig.cap = "Figure: Prevalence by ages (Zimbabwe smooth)"}
put(1,1, c(5,4,1,1))
...
```{r echo=FALSE, fig.height=5, fig.cap = "Figure: Prevalence by ages over time"}
